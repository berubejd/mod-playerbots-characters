...
- `{equipment}` — dynamic single-line equipment description. Rarity adjectives and weapon names are included only for rare+ quality items; below rare only the type is shown (e.g. "a dagger" not "a common dagger"). When bags are at least ~40% full, a bag-space summary is also appended. Examples:
  - Full set: "You have fine equipment made of leather. Your main weapon is a rare dagger called Death's Sting. In your off-hand you wield a dagger. Your ranged weapon is a rare bow called Heartstriker."
