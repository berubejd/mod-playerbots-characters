...
Endpoints that read or modify character data (history, memories, relationships, data) work for characters on your account. Endpoints that require the character to be online (context, debug, whisper, trigger) return `404` with a clear error when the target is offline.

**Character cards are the exception:** card *reads* (`GET /api/cards` and `GET /api/char/:guid/card`) work for any character with a card row, so the viewer can browse every encountered character. Card *writes* (edit, pin/unpin, regenerate) still require ownership, and pinned cards are read-only.
...
Cards are the DB-canonical, structured identity of each character — a small set of optional persona fields (`premise`, `personality`, `values`, `background`, `speech_style`, `quirks`), one row per character in `mod_pbc_cards`. They are autogenerated on first contact and can be seeded/pinned from disk (`characters/<Name>.card.txt`). Rendering is deterministic: the model is only ever called at write time (generation/derivation), never at read/load time.

Authority and gating:
- **Reading is open.** Any authenticated account can read any character's card and list all cards (persona is public). The payload carries `owned`/`editable` flags so clients can gate controls.
- **Editing/pinning/regenerating require ownership** — the character must belong to your account.
- **Pinned cards are read-only.** A pinned card (disk/SQL `override`, or pinned via the API) rejects edits and regeneration with `403` until unpinned. Pinned ⇒ file/SQL is authoritative; unpinned ⇒ DB is authoritative and editable.

#### `GET /api/cards`

List every character that has a content-bearing card row (across all accounts).
...
- `provenance` — `generated`, `edited`, or `override`
- `pinned` — whether the card is currently read-only / disk-authoritative
- `owned` — whether the character belongs to the authenticated account
- `is_online` — whether the character is currently logged in (regeneration requires it)

#### `GET /api/char/:guid/card`

Returns a character's card: structured fields, the deterministically-rendered text, provenance, and gating flags.
...
```json
{
  ...
  "card": "You are Jane, a Night Elf Priest. ..."
}
```

- `has_card` — `true` when the card has renderable content
- `editable` — `true` only when `owned` and not `pinned`
- `card` — the rendered card text (identity variables substituted). For a character with no card row, an owner receives the rendered default description with `has_card:false` and empty `fields`.
- Returns `404` for a non-owner when the character has no content-bearing card.

#### `POST /api/char/:guid/card`

Edit one or more persona fields (owner only; rejected with `403` if the card is pinned). Only the provided string fields are changed; the first edit flips `provenance` to `edited`. Returns the updated card payload (same shape as the GET).

Body (any subset of the six fields):

```json
{
  "premise": "Certain she is the finest healer in Darnassus.",
  "quirks": "Names every potion."
}
```

- `400` — no recognized string fields were provided, or the edit would leave the card with no content
- `403` — the card is pinned (unpin first)

#### `POST /api/char/:guid/card/pin` · `POST /api/char/:guid/card/unpin`

Pin (make read-only) or unpin (make editable) a card. Owner only. Returns `{"ok": true, "pinned": true|false}`. Returns `404` if the character has no card row.

#### `POST /api/char/:guid/card/regenerate`

Queue a fresh autogeneration for the card (owner only). The character must be **online** and the card must not be pinned. The heavy LLM work runs on the background card worker; a `card` WebSocket event is emitted when it completes.

- `403` — the card is pinned
- `404` — the character is offline
- `409` — regeneration unavailable (autogen disabled, pinned, or already in progress)
...
| `card` | Character's card was created/edited/regenerated | `{"event":"card","guid":12345}` |
...
`thinks`, `relationship`, `memory`, `card`, `online`, `offline`, and `party` events are simple triggers — fetch updated data via the REST API when received.
