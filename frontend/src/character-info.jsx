...
import { fetchCard, fetchCardAdditions, fetchRelationships, fetchContext,
         editCardAddition, deleteCardAddition, editRelationship, deleteRelationship,
         formatApiError } from './api.js';
...
export default function CharacterInfo({ token, selectedGuid, nameColorMap, reloadKey = 0, onDesync }) {
  const [openSection, setOpenSection] = useState('context');
  const [cardData, setCardData] = useState(null);
  const [additionsData, setAdditionsData] = useState(null);
  ...
  // Edit modal state — works for both additions and relationships
  ...
  const loadSection = useCallback((section) => {
    ...
    switch (section) {
      case 'card':
        ...
        names.push('card');
        fetches.push(fetchCardAdditions(token, guid));
        ...
        results.forEach((result, i) => {
          if (result.status === 'fulfilled') {
            switch (names[i]) {
              ...
              case 'additions': setAdditionsData(result.value); break;
  ...
  // --- Card addition edit/delete handlers ---

  const handleAdditionEditOpen = useCallback((id, text) => {
    setEditModal({ show: true, type: 'addition', id, name: null, text });
  }, []);

  const handleAdditionDeleteOpen = useCallback((id, text) => {
    setDeleteModal({ show: true, type: 'addition', id, name: null, text });
  }, []);
  ...
  const handleEditSave = useCallback(async (newText) => {
    try {
      if (editModal.type === 'addition') {
        await editCardAddition(token, selectedGuid, editModal.id, newText, editModal.text);
        // Update local state
        setAdditionsData(prev => {
          if (!prev || !prev.additions) return prev;
          return {
            ...prev,
            additions: prev.additions.map(a =>
              a.id === editModal.id ? { ...a, text: newText } : a
            ),
          };
        });
        toast('Addition updated', 'success');
      } else if (editModal.type === 'relationship') {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    try {
      if (deleteModal.type === 'addition') {
        await deleteCardAddition(token, selectedGuid, deleteModal.id, deleteModal.text);
        // Update local state and re-index remaining additions.
        // The backend uses 1-based array indices as IDs, so after an element
        // is erased all subsequent items shift down by one position.
        ...
        setAdditionsData(prev => {
          ...
          return {
            ...
            additions: prev.additions
              ...
              .map(a => (a.id > deletedId ? { ...a, id: a.id - 1 } : a)),
          };
        });
        toast('Addition deleted', 'success');
      } else if (deleteModal.type === 'relationship') {
        ...
        <>
          {loading.card ? renderLoading('card') : (
            <>
              ...
              {loading.card ? null : renderError('additions')}
              {additionsData && additionsData.additions && additionsData.additions.length > 0 && (
                <>
                  {additionsData.additions.map((add, index) => (
                    <div key={add.id} class="message-line mb-2 position-relative">
                      <div class="message-actions position-absolute top-0 end-0" style="z-index: 1;">
                        <button
                          class="btn btn-sm p-0 px-1"
                          title="Edit"
                          onClick={(e) => { e.stopPropagation(); handleAdditionEditOpen(add.id, add.text); }}
                        >
                        ...
                        <button
                          class="btn btn-sm p-0 px-1"
                          title="Delete"
                          onClick={(e) => { e.stopPropagation(); handleAdditionDeleteOpen(add.id, add.text); }}
                        >
                          <i class="bi bi-trash3"></i>
                        </button>
                      </div>
                      <div class="action-menu position-absolute top-0 end-0">
                        <ActionMenu onEdit={() => handleAdditionEditOpen(add.id, add.text)} onDelete={() => handleAdditionDeleteOpen(add.id, add.text)} />
                      </div>
                      <span class="small fw-bold">Addition #{index + 1}</span>
                      <div class="small text-body-secondary" style="white-space: pre-wrap; word-break: break-word;">
                        <FormattedText text={add.text} />
                      </div>
                    </div>
                  ))}
                </>
              )}
              {!cardData && !additionsData && !errors.card && !errors.additions && (
