...
Each character's identity lives in a structured **character card** — a small set of optional persona fields (premise, personality, values, background, speech style, quirks) stored in the database and keyed by character GUID. On first contact a lore-grounded card is autogenerated and persisted; you can also seed and **pin** cards from disk (`characters/<Name>.card.txt`), in which case the file stays authoritative. Cards can be browsed (for every character you've encountered), edited, pinned/unpinned, and regenerated from the web app or via `.chars` commands. Rendering is deterministic — the model is only ever called when a card is generated or derived, never at read time.
...
Character cards are autogenerated on first contact and stored in the database. The relevant options live in the **CHARACTER CARDS** section of the config:

- **`PBC.CardAutogenEnabled`** — generate a lore-grounded card the first time a character with no card is encountered. With this off, characters without a card fall back to `PBC.DefaultCharacterDescription`.
- **`PBC.CardGenerationFormat`** — how the model returns the structured fields: `json` (recommended for cloud models) or `labeled` (more robust on small local models).
- **`PBC.CardGenerationConnection`** — route the one-time generation/derivation calls to a dedicated (more capable) connection; falls back to `PBC.DefaultConnection` when empty.
- **`PBC.CardDeriveOnImport`** — after importing a disk card, derive the remaining empty fields from the authored background (the authored text itself is never altered).
- **`PBC.CardGenerationTemperature`** / **`PBC.CardDerivationTemperature`** — sampling temperatures for generation (hotter, for distinct characters) and derivation (cooler, constrained by authored fields).

Disk cards (`characters/<Name>.card.txt`) are imported on startup and `.chars reload`. A new or changed file is imported as a **pinned** card: while pinned the file stays authoritative (re-imported on content change) and the card is read-only. Unpin it (via the web app or `.chars unpin`) to make the database copy authoritative and editable. See [Usage](#usage) for authoring tips.

Optionally, an AI **mood** pass can refine the cheap, model-free mood that every event is stamped with — see the **MEMORY MOOD** section (`PBC.MoodEnabled`, `PBC.MoodRefineScope`). The current mood is exposed to prompts as the `{mood}` variable.

### HTTP Server & Web App

![PBC Web App](https://d7.wtf/s/pbc-web.png)

The module can optionally run a built-in HTTP/WS server with a web frontend, providing an interface for managing the characters, as well as an API for external tools and integrations. The web interface supports viewing and editing chat history, relationships, memories, and character cards — including browsing the cards of every character you've encountered and editing/pinning/regenerating your own. This is disabled by default and could be enabled in the config, read the `HTTP SERVER` section and follow the comments there.
