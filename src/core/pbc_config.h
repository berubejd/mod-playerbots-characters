...
struct PBC_EventItem
{
    ...
    // GUIDs of real (non-bot) players in the group whose characters should
    // receive history passively (no LLM calls).  When PBC.TrackPlayerCharacter
    // is enabled, these players are added to silentCharGuids as well so the
    // normal history-write path handles them.  This separate list exists so
    // secondary events can propagate replies to player characters without
    // re-writing histLine (players are treated like replyOnlyCharGuids).
    std::vector<uint64_t> playerCharGuids;

    // For whisper events: the name of the player who sent the whisper and the
    // name of the bot target.
...
struct PBC_PendingEventRequest
{
    ...
    // GUIDs of real (non-bot) players whose characters should receive new
    // replies from this secondary event (but NOT histLine, which was already
    // written by the primary event).
    std::vector<uint64_t> playerCharGuids;
};
