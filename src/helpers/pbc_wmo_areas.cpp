...
uint32_t PBC_LoadWMOAreaNames()
{
    ...
    if (!file.is_open())
    {
        PBC_Log(PBC_LogLevel::PBC_WARNING, "Could not open WMOAreaTable.dbc at '{}' — WMO area names will not be available.", dbcPath);
        return 0;
    }
    ...
    {
        PBC_Log(PBC_LogLevel::PBC_ERROR, "WMOAreaTable.dbc has invalid magic — skipping.");
        return 0;
    }
    ...
    {
        PBC_Log(PBC_LogLevel::PBC_ERROR, "WMOAreaTable.dbc has unexpected field count {} (expected {}) — skipping.", fieldCount, WMO_DBC_FIELD_COUNT);
        return 0;
    }

    if (recordSize != WMO_DBC_RECORD_SIZE)
    {
        PBC_Log(PBC_LogLevel::PBC_ERROR, "WMOAreaTable.dbc has unexpected record size {} (expected {}) — skipping.", recordSize, WMO_DBC_RECORD_SIZE);
        return 0;
    }
    ...
    if (!file)
    {
        PBC_Log(PBC_LogLevel::PBC_ERROR, "WMOAreaTable.dbc read error — skipping.");
        return 0;
    }
    file.close();
    ...
    PBC_Log(PBC_LogLevel::PBC_DEFAULT, "Loaded {} WMO area names from WMOAreaTable.dbc.", loaded);
    return loaded;
}
